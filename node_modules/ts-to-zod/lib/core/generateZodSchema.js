 ...
 * ```
 */
function generateZodSchemaVariableStatement({ node, sourceFile, varName, zodImportValue = "z", getDependencyName = (identifierName) => case_1.camel(`${identifierName}Schema`), }) {
    let schema;
    const dependencies = [];
    ...
    return {
        dependencies: uniq_1.default(dependencies),
...
function buildZodProperties({ members, zodImportValue: z, sourceFile, dependencies, getDependencyName, }) {
    ...
    members.forEach((member) => {
        ...
        const jsDocTags = jsDocTags_1.getJSDocTags(member, sourceFile);
...
function buildZodPrimitive({ z, typeNode, isOptional, isPartial, isRequired, jsDocTags, sourceFile, dependencies, getDependencyName, }) {
    const zodProperties = jsDocTags_1.jsDocTagToZodProperties(jsDocTags, isOptional, Boolean(isPartial), Boolean(isRequired));
    ...
    if (ts.isTypeReferenceNode(typeNode) && ts.isIdentifier(typeNode.typeName)) {
        ...
        if (["Omit", "Pick"].includes(identifierName) && typeNode.typeArguments) {
            ...
            return f.createCallExpression(f.createPropertyAccessExpression(buildZodPrimitive({
                ...
                dependencies,
                getDependencyName,
            }), f.createIdentifier(case_1.lower(identifierName))), undefined, [parameters]);
        }
    ...
    if (ts.isUnionTypeNode(typeNode)) {
        const values = typeNode.types.map((i) => buildZodPrimitive({
            z,
            typeNode: i,
            isOptional: false,
            jsDocTags: {},
 ...
 */
function buildZodObject({ typeNode, z, dependencies, sourceFile, getDependencyName, baseSchema, }) {
    ...
    if (properties.length > 0) {
        ...
        objectSchema = buildZodSchema(baseSchema || z, baseSchema ? "extend" : "object", [
